...
public:
    ...
    void run(int n_in,double rate_in,double margin_in,int method_in)
    {
        ...
        for (int i=0; i<relation_num; i++)
        {
            A[i].resize(n);
            for (int j=0; j<n; j++)
                A[i][j] = common::randn(0,1.0/n,-1,1);
            norm2one(A[i]);
        }
        ...
        for (int i=0; i<relation_num; i++)
        {
            for (int ii=0; ii<n; ii++)
                relation_vec[i][ii] = common::randn(0,1.0/n,-1,1);
        }
        for (int i=0; i<entity_num; i++)
        {
            for (int ii=0; ii<n; ii++)
                entity_vec[i][ii] = common::randn(0,1.0/n,-1,1);
        }
...
void prepare()
{
    ...
    for (int i=0; i<relation_num; i++)
    {
        ...
        for (map<int,vector<int> >::iterator it = left_entity[i].begin(); it!=left_entity[i].end(); it++)
        {
            sum1++;
            sum2+=it->second.size();
            sum3+=common::sqr(it->second.size());
        }
        left_mean[i]=sum2/sum1;

        left_var[i]=sum3/sum1-common::sqr(left_mean[i]);
    }
    for (int i=0; i<relation_num; i++)
    {
        double sum1=0,sum2=0,sum3=0;
        ...
        right_var[i]=sum3/sum1-common::sqr(right_mean[i]);
    }
